...
class sleep_pretrain(nn.Module):
    ...
    def training_epoch_end(self,outputs):
        ...
        fusion_loss = torch.hstack([torch.tensor(x) for x in outputs['fusion_loss']]).mean()
        spect_loss = torch.hstack([torch.tensor(x) for x in outputs['spect_loss']]).mean()
        intra_loss = torch.hstack([torch.tensor(x) for x in outputs['intra_loss']]).mean()
    ...
    def on_epoch_end(self):
        ...
        torch.save(full_chkpoint, os.path.join(self.config.exp_path, self.name+"_full"+'.pt'))
        return None

    def ft_fun(self,file_name,epoch,train_idx,val_idx,split):
        src_path = self.config.src_path
        train_dl,valid_dl= cross_data_generator(src_path,train_idx,val_idx,self.config)
        sleep_eval = sleep_ft(self.config.exp_path+"/"+self.name+'.pt',self.config,train_dl,valid_dl,epoch,self.loggr)
    ...
    def fit(self):
        ...
        for epoch in range(self.epochs):
            ...
            outputs = {'loss':[],'time_loss':[],'fusion_loss':[],'spect_loss':[],'intra_loss':[]}

            self.model.train()
            for batch_idx,batch in enumerate(self.dataloader):
                loss,time_loss,fusion_loss,spect_loss,intra_loss= self.training_step(batch,batch_idx)
            ...
            if (epoch%4==0) and (epoch>=80): 
                ...
                if self.max_f1<f1:
                    ...
                    torch.save(chkpoint, os.path.join(self.config.exp_path, self.name+'_best.pt'))
                    self.max_f1,self.max_kappa,self.max_bal_acc,self.max_acc = f1,kappa,bal_acc,acc

                if self.max_mean_f1<mean_f1:
                    chkpoint = {'eeg_model_state_dict':self.model.model.eeg_encoder.state_dict(),'best_pretrain_epoch':epoch}
                    torch.save(chkpoint, os.path.join(self.config.exp_path, self.name+'_mean_best.pt'))
